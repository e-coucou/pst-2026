...
- [x] Script exécuté dans Supabase par l'utilisateur (plusieurs fois, au fil des correctifs — colonnes de rang manquantes puis bug de séquence `teams.id`/`games.id` sans `DEFAULT`, voir §15).
- [x] UI écrite : `/live/archive` ("Archiver le tournoi", appelle `archive_tournament` puis `POST /api/admin/recompute-elo`, avec retry dédié si le recalcul échoue après un archivage déjà réussi) et `/live/next-season` ("Passer à la saison suivante", appelle `advance_to_next_season`, bloquée tant que la saison active n'est pas archivée). Liens dans `/live/super` (section "Fin de saison").
- [ ] Test sur données jetables (année factice) **sauté en pratique** — l'archivage réel de 2026 a servi de premier test grandeur nature (deux échecs rencontrés et corrigés en conditions réelles, sans conséquence sur les données grâce à l'atomicité des fonctions Postgres, cf. §15). Choix assumé rétrospectivement plutôt que planifié.
- [x] **Constat confirmé par l'utilisateur** : le tournoi live `status='TERMINE'`/`format='10_equipes'` observé le 2026-08-05 était bien le vrai tournoi du 4 août, pas un résidu de test.
- [x] Archivage réel de la saison 2026 exécuté et vérifié (`/tournois/2026`, classement Dynamique cohérent, cf. §14/§15).
- [ ] Action 2 ("Passer à la saison suivante") **pas encore déclenchée** — à la discrétion du super, aucune urgence tant que le direct doit rester visible.
