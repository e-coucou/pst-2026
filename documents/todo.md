...
- [x] **Résidence :** Documents — abandon de l'upload binaire Supabase Storage au profit de liens Google Drive (`residence_documents.external_url`) : certains PDF de la résidence dépassent 50 Mo, hors de portée pratique du quota gratuit Supabase (1 Go, déjà partagé avec `photos_import`). Ajout d'une catégorisation (Syndic, Fournisseurs, AG, PV, Autre — filtrable par pills) et d'une visibilité **public/privé** par document (toggle globe/cadenas). Les documents publics sont consultables par tout membre connecté (pas seulement super) sur une nouvelle page `/render/documents`, via une 2ème policy RLS combinée en OR avec l'accès total `super`. Nouveau bouton "Documents" toujours visible sur `/render` (à côté du bouton "Accès réservé" réservé au super). **Nécessite l'exécution manuelle du SQL fourni dans Supabase** (`documents/private/migrate_documents_to_drive.sql` puis `add_document_category_visibility.sql`).
