...
- [ ] **Sécurité :** Réinitialisation de mot de passe (`/reset-password`, comptes pseudo `@pst.net`) vérifiée par le seul code d'invitation — ce code est **partagé par tous les membres**, donc n'importe quel membre connaissant le pseudo d'un autre peut réinitialiser son mot de passe (contrairement à l'usage du code à l'inscription, qui ne fait qu'autoriser la création d'un compte pour soi-même). Risque accepté vu le contexte (club fermé) ; log d'audit `session_logs`/`PASSWORD_RESET` en place pour tracer un usage suspect via `/live/activity`. Pas de rate-limiting. À revoir si le club grossit ou si un abus est constaté.
