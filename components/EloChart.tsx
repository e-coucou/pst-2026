...
const METHOD_COLOR: Record<'pst' | 'modern' | 'skill', string> = {
  pst: '#dc2626',
  modern: '#a855f7',
  skill: '#10b981',
}
...
export default function EloChart({ history }: { history: any[] }) {
  const [method, setMethod] = useState<'pst' | 'modern' | 'skill'>('pst')
  ...
  const data = useMemo(() => {
    return history?.map((h, index) => ({
      ...
      skill: h.skill_ordinal != null ? parseFloat(h.skill_ordinal.toFixed(1)) : null,
      year: h.year,
      gameId: h.game_id
    })) || []
  }, [history])
    ...
    <div className="w-full h-full min-h-[300px] min-w-0 flex flex-col gap-6">
      ...
      <div className="flex gap-3">
        ...
        <button
          ...
          title="Modèle bayésien (openskill) — pensé pour peu de matchs et des doublettes qui changent chaque saison"
          className={`px-6 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${
            method === 'skill'
