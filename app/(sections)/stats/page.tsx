...
import {
  ...
  TrendingUp, BarChart3, ChevronRight, Zap, Search,
        ...
        {activeTab === 'calibration' && (
          <div className="space-y-6 animate-in fade-in zoom-in-95 duration-300">
            <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest max-w-2xl">
              ...
              que donnerait le cœur de quatre moteurs — <span className="text-red-500">Classic</span> (écart
              d&apos;ELO rugby-inspiré), <span className="text-purple-400">Modern</span> (écart d&apos;ELO FIDE),{' '}
              <span className="text-emerald-400">Dynamique</span> (openskill, incertitude propre à chaque
              joueur) et <span className="text-amber-500">Combiné</span> (régression logistique sur les 3
              écarts, validée en leave-one-out) — sans le bonus de forme du jour ni l&apos;explosivité,
            ...
            <div className="grid grid-cols-2 md:grid-cols-5 gap-4">
              ...
              <StatCard label="Précision Combiné" value={`${calibration.combined.accuracy.toFixed(0)}%`} icon={<Target size={20} />} color="text-amber-500" />
              ...
              <StatCard label="Brier Combiné" value={calibration.combined.brierScore.toFixed(3)} icon={<TrendingUp size={20} />} color="text-amber-500" />
            </div>
            ...
            <div className="bg-zinc-900/50 border border-white/5 p-6 rounded-[2.5rem]">
              ...
              <CalibrationChart
                ...
                combinedBuckets={calibration.combined.buckets}
              />
              <p className="text-[10px] text-zinc-500 mt-6 font-bold uppercase italic text-center">
                Score de Brier : 0 = parfait, 0,25 = équivalent à toujours prédire 50/50. Combiné
                n&apos;est pas plus performant que Classic seul sur ce volume de données (145
                matchs) — affiché pour la transparence de la comparaison.
              </p>
            </div>
          </div>
        )}
