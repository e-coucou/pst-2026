	...
	import { getRequiredCount, getTeamIds, getGassinIds, generateRoundRobinPairs, POULE5_COURTS, buildTeamsPayload } from '@/utils/live-teams';
	...
	export default function LiveAdminWizard() {
	 ...
	 const syncTeamsToDatabase = async (pList: any[], tList: any[]) => {
	   ...
	   await supabase.from('live_teams').upsert(buildTeamsPayload(pList, tList, format));
	 };
	...
	const handlePairComplete = async (pointeur: any, tireur: any) => {
	 ...
	 const confirmAndCreateTournament = async () => {
	   ...
	   try {
	     ...
	     const { error: teamsError } = await supabase.from('live_teams').insert(buildTeamsPayload(draftP, draftT, format));
