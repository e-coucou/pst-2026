...
export default function LiveAdminWizard() {
  ...
  useEffect(() => {
    async function init() {
      ...
      if (existing && existing.length > 0) {
        const ps = existing.filter(x => x.role === 'Pointeur').map(x => ({ id: x.player_id, nom: x.nom, elo: x.elo_at_selection }));
        const ts = existing.filter(x => x.role === 'Tireur').map(x => ({ id: x.player_id, nom: x.nom, elo: x.elo_at_selection }));
  ...
  const fetchPlayersWithElo = async () => {
    ...
    const { data: elos } = await supabase.from('elo_history').select('player_id, elo_value').order('created_at', { ascending: false });

    if (profiles && elos) {
      const playersDetailed = profiles.map(p => {
        const lastElo = elos.find(e => e.player_id === p.id);
        return { ...p, elo: lastElo ? lastElo.elo_value : 100 };
  ...
  const finalizeSelectionAndSave = async () => {
    ...
    try {
      ...
      const toInsert = [
        ...selectedPointeurs.map(p => ({ player_id: p.id, role: 'Pointeur', elo_at_selection: p.elo, nom: p.nom })),
        ...selectedTireurs.map(t => ({ player_id: t.id, role: 'Tireur', elo_at_selection: t.elo, nom: t.nom }))
      ];
  ...
  const confirmAndCreateTournament = async () => {
    ...
    try {
      ...
      const gen = (ids: string[], village: string) => {
        for (let i = 0; i < ids.length; i++) {
          for (let j = i + 1; j < ids.length; j++) {
            pouleMatches.push({
              poule: village, type: 'Poule', tableau: 'Principal',
              team1_id: ids[i], team2_id: ids[j], status: 'EN_ATTENTE'
            });
          }
        }
      };
        ...
        {step === 1 ? (
                  ...
                  return (
                    <div key={p.id} className={`p-3 rounded-2xl border transition-all ${isP || isT ? 'opacity-20 bg-black' : 'bg-zinc-900 border-white/5 hover:border-red-600'}`}>
                      <div className="flex justify-between items-center">
                        <span className="font-bold text-[11px] uppercase">{p.nom} <span className="text-zinc-600 ml-2">{p.elo}</span></span>
          ...
          <div className="space-y-12">
            <div className="flex justify-between items-end">
               <div>
                  <h2 className="text-3xl font-black italic uppercase">Ajustement des Duos</h2>
